...
def gui_create_model(database):
    ...
    info_list = QStandardItemModel()
    info_list.setHorizontalHeaderLabels(['Имя Клиента', 'IP Адрес', 'Порт', 'Время подключения'])
    for row in list_users:
        ...
        info_list.appendRow([user, ip, port, time])
    return info_list
...
def create_stat_model(database):
    ...
    for row in hist_list:
        ...
        hist_list.appendRow([user, last_seen, sent, recvd])
    return hist_list
