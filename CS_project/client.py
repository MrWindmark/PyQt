...
def arg_parser():
    ...
    srvr_addr = namespace.addr
    srvr_port = namespace.port
    clnt_name = namespace.name
    clnt_pwd = namespace.password

    # проверим подходящий номер порта
    if not 1023 < srvr_port < 65536:
        logger.critical(
            f'Попытка запуска клиента с неподходящим номером порта: {srvr_port}.'
            f'Допустимы адреса с 1024 до 65535. Клиент завершается.')
        exit(1)

    return srvr_addr, srvr_port, clnt_name, clnt_pwd
